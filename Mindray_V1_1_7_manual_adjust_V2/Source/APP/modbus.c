    ...
    {   
        case 0x50:
            ...
            if((CALIB_CAPMAX_FLAG | CALIB_CAPMIN_FLAG) == (RunVarParam.CalibFlag & (CALIB_CAPMAX_FLAG | CALIB_CAPMIN_FLAG)))                      //高低液位值都得到了
            {
                ...
                {
                    AddrOffset = (UserParam.LiquidType - 1) * 24;                //地址随液体类型偏移
                    for(i = 0; i < 4; i ++)
                    {                                                           //放入缓存
                    ...
                    RunVarParam.AdjustCap = 0;
                    AddrOffset = (UserParam.LiquidType - 1) * 4;                //地址随液体类型偏移
                    for(i = 0; i < 4; i ++)
                    {                                                           //放入缓存
                        Cur_Param[CAP_ADJUST1 + AddrOffset + i] = (uint8_t)(RunVarParam.AdjustCap >> (3 - i) * 8);
                    }                                                           //写进EEPROM
//                    ProductParam.EepromWrEn = EEPROM_WR_ENABLE;
                    Eeprom_Write3T_MultiBytes(CAP_ADJUST1 + AddrOffset, (uint8_t const *)&Cur_Param[CAP_ADJUST1 + AddrOffset], 4);    
//                    ProductParam.EepromWrEn = EEPROM_WR_DISABLE;
        ...
        case 0x55:
            if((RcvBuf[4] == 0x00) && (RcvBuf[5] == 0x00))                      //标定40mm刻度液位
            {
                ProductParam.Cap40mm = RunVarParam.CapBak;
                RunVarParam.CalibFlag |= CALIB_CAP40MM_FLAG;
            }
            else if((RcvBuf[4] == 0xFF) && (RcvBuf[5] == 0x00))                 //标定80mm刻度液位
            {
                ProductParam.Cap80mm = RunVarParam.CapBak;
                RunVarParam.CalibFlag |= CALIB_CAP80MM_FLAG;
            }
            if((CALIB_CAP40MM_FLAG | CALIB_CAP80MM_FLAG) == (RunVarParam.CalibFlag & (CALIB_CAP40MM_FLAG | CALIB_CAP80MM_FLAG)))                      //上/下刻度液位值都得到了
            {
                if(ProductParam.Cap40mm < ProductParam.Cap80mm)
                {
                    AddrOffset = (UserParam.LiquidType - 1) * 24;                //地址随液体类型偏移
                    for(i = 0; i < 4; i ++)
                    {                                                           //放入缓存
                        Cur_Param[CAP40MM1 + AddrOffset + i] = (uint8_t)(ProductParam.Cap40mm >> (3 - i) * 8);
                        Cur_Param[CAP80MM1 + AddrOffset + i] = (uint8_t)(ProductParam.Cap80mm >> (3 - i) * 8);
                    }                                                           //写进EEPROM，获得电容量程
                    Eeprom_Write3T_MultiBytes(CAP40MM1 + AddrOffset, (uint8_t const *)&Cur_Param[CAP40MM1 + AddrOffset], 4);
                    Eeprom_Write3T_MultiBytes(CAP80MM1 + AddrOffset, (uint8_t const *)&Cur_Param[CAP80MM1 + AddrOffset], 4);
                    RunVarParam.CalibFlag &= ~(CALIB_CAP40MM_FLAG | CALIB_CAP80MM_FLAG);
                }
                else
                {
                    MBASCII_SendErr(MB_EX_CALIBRATION_ERR);
    ...
    if((RcvBuf[4] == 0xFF) && (RcvBuf[5] == 0x00))
    {
        ...
        if(RunVarParam.EmptyCap < ProductParam.CapMin)
        {
            if((abs((int32_t)RunVarParam.EmptyCap - C_Current) 
               ...
               < (abs(RunVarParam.AdjustCap) - ((ProductParam.CapMax - ProductParam.CapMin) / (ProductParam.HeightRange * 10)))))
            {
                RunVarParam.AdjustCap = (int32_t)RunVarParam.EmptyCap - C_Current;
                AddrOffset = (UserParam.LiquidType - 1) * 4;                //地址随液体类型偏移
                for(i = 0; i < 4; i ++)
                {                                                           //放入缓存
                    Cur_Param[CAP_ADJUST1 + AddrOffset + i] = (uint8_t)(RunVarParam.AdjustCap >> (3 - i) * 8);
...
void MBASCII_Fun26(void)
{
    ...
    for (long k = 0; k < Register_Num; ReadAdrL += 2, k += 2)
    {
        ...
        case 0xC4:
          //电容40mm刻度标定高度值
          fBuf = (float)ProductParam.Cap40mmHeight;
          DataBuf = *(uint32_t *)&fBuf;
          break;
          
        case 0xC6:
          //电容80mm刻度标定高度值
          fBuf = (float)ProductParam.Cap80mmHeight;
          DataBuf = *(uint32_t *)&fBuf;
          break;
          
        case 0xC8:
          //电容40mm刻度标定电容值
          fBuf = (float)ProductParam.Cap40mm;
          DataBuf = *(uint32_t *)&fBuf;
          break;
          
        case 0xCA:
          //电容80mm刻度标定电容值
          fBuf = (float)ProductParam.Cap80mm;
          DataBuf = *(uint32_t *)&fBuf;
          break;   
          
        case 0xCC:
          //校准后的空电容值差值
          fBuf = (float)RunVarParam.AdjustCap;
          DataBuf = *(uint32_t *)&fBuf;
          break;   
...
void MBASCII_Fun27(void)
{
    ...
    case 0xC4:
      //电容40mm刻度标定高度
        fbuf = HexToFloat(&RcvBuf[7 + index]);
        ProductParam.Cap40mmHeight = (uint16_t)fbuf;
        AddrOffset = (UserParam.LiquidType - 1) * 8;                //地址随液体类型偏移
        Cur_Param[CAP40MMHEIGHT1 + AddrOffset] = (uint16_t)fbuf >> 8;
        Cur_Param[CAP40MMHEIGHT1 + AddrOffset + 1] = (uint16_t)fbuf & 0xFF;
        Eeprom_Write3T_MultiBytes(CAP40MMHEIGHT1 + AddrOffset, 
                                  (uint8_t const *)&Cur_Param[CAP40MMHEIGHT1 + AddrOffset], 2);
      break;
      
    case 0xC6:
      //电容80mm刻度标定高度
        fbuf = HexToFloat(&RcvBuf[7 + index]);
        ProductParam.Cap80mmHeight = (uint16_t)fbuf;
        AddrOffset = (UserParam.LiquidType - 1) * 8;                //地址随液体类型偏移
        Cur_Param[CAP80MMHEIGHT1 + AddrOffset] = (uint16_t)fbuf >> 8;
        Cur_Param[CAP80MMHEIGHT1 + AddrOffset + 1] = (uint16_t)fbuf & 0xFF;
        Eeprom_Write3T_MultiBytes(CAP80MMHEIGHT1 + AddrOffset, 
                                  (uint8_t const *)&Cur_Param[CAP80MMHEIGHT1 + AddrOffset], 2);
      break;

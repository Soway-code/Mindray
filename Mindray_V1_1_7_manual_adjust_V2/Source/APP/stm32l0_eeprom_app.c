...
const uint8_t User_Default_Param[PRO_DEFAULT_LEN] = 
{
    ...
    0x00,0x28,                                                             //电容40刻度标定高度1
    0x00,0x50,                                                             //电容80刻度标定高度1
    0x00,0x70,                                                             //电容上刻度标定高度1
    0x00,0x0A,                                                             //电容下刻度标定高度2
    0x00,0x28,                                                             //电容40刻度标定高度2
    0x00,0x50,                                                             //电容80刻度标定高度2
    0x00,0x70,                                                             //电容上刻度标定高度2
    0x00,0x0A,                                                             //电容下刻度标定高度3
    0x00,0x28,                                                             //电容40刻度标定高度3
    0x00,0x50,                                                             //电容80刻度标定高度3
    0x00,0x70,                                                             //电容上刻度标定高度3
    0x00,0x0A,                                                             //电容下刻度标定高度4
    0x00,0x28,                                                             //电容40刻度标定高度4
    0x00,0x50,                                                             //电容80刻度标定高度4
    0x00,0x70,                                                             //电容上刻度标定高度4
    0x00,0x0A,                                                             //电容下刻度标定高度5
    0x00,0x28,                                                             //电容40刻度标定高度5
    0x00,0x50,                                                             //电容80刻度标定高度5
...
void Read_Device_Param(void)
{
    ...
    ProductParam.CapMin = HexToUlong(&Cur_Param[CAPMIN1 + (UserParam.LiquidType - 1) * 24]);
    //电容下刻度
    ProductParam.CapLow = HexToUlong(&Cur_Param[CAPLOW1 + (UserParam.LiquidType - 1) * 24]);
    //电容40mm刻度
    ProductParam.Cap40mm = HexToUlong(&Cur_Param[CAP40MM1 + (UserParam.LiquidType - 1) * 24]);
    //电容80刻度
    ProductParam.Cap80mm = HexToUlong(&Cur_Param[CAP80MM1 + (UserParam.LiquidType - 1) * 24]);
    //电容上刻度
    ProductParam.CapHigh = HexToUlong(&Cur_Param[CAPHIGH1 + (UserParam.LiquidType - 1) * 24]);
    //电容满点
    ProductParam.CapMax = HexToUlong(&Cur_Param[CAPMAX1 + (UserParam.LiquidType - 1) * 24]);
    
    //电容下刻度标定高度
    ProductParam.CapLowHeight = (uint16_t)(Cur_Param[CAPLOWHEIGHT1 + (UserParam.LiquidType - 1) * 8] << 8) 
                                | Cur_Param[CAPLOWHEIGHT1 + (UserParam.LiquidType - 1) * 8 + 1];
    //电容上刻度标定高度
    ProductParam.CapHighHeight = (uint16_t)(Cur_Param[CAPHIGHHEIGHT1 + (UserParam.LiquidType - 1) * 8] << 8) 
                                | Cur_Param[CAPHIGHHEIGHT1 + (UserParam.LiquidType - 1) * 8 + 1];
    //电容下刻度标定高度
    ProductParam.Cap40mmHeight = (uint16_t)(Cur_Param[CAP40MMHEIGHT1 + (UserParam.LiquidType - 1) * 8] << 8) 
                                | Cur_Param[CAP40MMHEIGHT1 + (UserParam.LiquidType - 1) * 8 + 1];
    //电容上刻度标定高度
    ProductParam.Cap80mmHeight = (uint16_t)(Cur_Param[CAP80MMHEIGHT1 + (UserParam.LiquidType - 1) * 8] << 8) 
                                | Cur_Param[CAP80MMHEIGHT1 + (UserParam.LiquidType - 1) * 8 + 1];
    ...
    RunVarParam.AdjustCap = HexToUlong(&Cur_Param[CAP_ADJUST1 + (UserParam.LiquidType - 1) * 4]);
}

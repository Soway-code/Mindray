...
void MBASCII_Fun2B(void)
{
  ...
  SenBuf[SendLen++] = (MBASCII_GetSlaveAddr(RcvBuf)) ? UsartParam.DeviceAddr : 0x00;
  SenBuf[SendLen++] = 0x2B;
  SenBuf[SendLen++] = RcvBuf[4];
  SenBuf[SendLen++] = RcvBuf[5];
  if(!((RcvBuf[2] == UsartParam.DeviceAddr)
       &&(ReadAdr >= ASCII_PRODUCTION_INF_BEGIN)
       &&(ReadAdr <= ASCII_PRODUCTION_INF_END)
       &&((ReadAdr + Register_Num) <= (ASCII_PRODUCTION_INF_END + 1))
  ...
  for(i = 0; i < Register_Num; i++)
  {
    ...
    case 0x00E0:
      ...
      Eeprom_Read_MultiBytes((ORGANIZATION + 1), &SenBuf[SendLen], objlen);
      SendLen += objlen;
      break;
    //产品代号  
    case 0x00E1:
      ...
      Eeprom_Read_MultiBytes((PRODUCTION + 1), &SenBuf[SendLen], objlen);
      SendLen += objlen;
      break;
    //硬件版本  
    case 0x00E2:
      ...
      Eeprom_Read_MultiBytes((HARDWAREVER + 1), &SenBuf[SendLen], objlen);
      SendLen += objlen;
      break;
    //软件版本 
    case 0x00E3:
      ...
      {
        SenBuf[SendLen++] = SensorSoftVersion[objlen + 1];
      }
      break;
    //设备ID  
    case 0x00E4:
      ...
      Eeprom_Read_MultiBytes((DEVICENUM + 1), &SenBuf[SendLen], objlen);
      SendLen += objlen;
      break;
    //客户编码  
    case 0x00E5:
      ...
      Eeprom_Read_MultiBytes((CUSTOMER + 1), &SenBuf[SendLen], objlen);
      SendLen += objlen;
      break;
...
void ModbusAutoUpload(void)
{
     ...
     if(RunVarParam.CapBak < 0x00FFFF00)       
      MBASCII_SendMsg(SenBuf,SenBufLen);   //Modbus发送消息 
